...
internal static partial class AniListPlugin {
    ...
    private static async Task<IReadOnlyList<Media>> SearchAsync(string search, int? year, int limit) { var data = await GraphQlAsync<SearchData>(SearchQuery, new { search, year, limit }); return data.Page?.Media ?? []; }
    ...
    private static readonly string SearchQuery = $"query ($search: String!, $year: Int, $limit: Int!) {{ Page(perPage: $limit) {{ media(search: $search, seasonYear: $year, type: ANIME, isAdult: false, sort: [SEARCH_MATCH, POPULARITY_DESC]) {{ {BasicFields} }} }} }}";
