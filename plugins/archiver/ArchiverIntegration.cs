    ...
    internal async Task<object> DispatchAsync(IntegrationRequest request, CancellationToken cancellationToken) {
        ...
        switch (request.Operation) {
            case IntegrationOperations.Probe:
                return new ProbeResult(system.InstanceId, "The Archiver", system.ApplicationVersion, Capabilities);
            case IntegrationOperations.Inspect: {
                var input = Input<InspectTransferInput>(request);
                var kind = input.EntityKind switch { MediaKinds.Book => ArchiverWire.Book, MediaKinds.Comic => ArchiverWire.Comic, _ => throw new IntegrationFailure("This executor profile supports books and comics.") };
                ...
                var formats = kind == ArchiverWire.Book ? new[] { ArchiverWire.Epub, ArchiverWire.Pdf } : [ArchiverWire.Cbz];
            ...
            case IntegrationOperations.Submit: {
                ...
                if (pinned is null || pinned.Format is not (ArchiverWire.Epub or ArchiverWire.Pdf or ArchiverWire.Cbz) || input.ItemIds.Count != 1 || input.MaximumItems != 1)
                ...
                return await client.SendAsync<JobSnapshot>(HttpMethod.Post, "jobs", new SubmitJob(input.ClientOperationId,
                    new(input.Url), new(pinned.Id, input.SelectionRevision, input.ItemIds), new(ArchiverWire.Profile, pinned.Format), new(input.MaximumItems, input.MaximumBytes)),
