...
The plugin searches existing comic runs, reads exact issue-to-file associations, and lists existing root folders. It preserves fractional and special issue labels, distinguishes Comic Vine run identities (`4050-…`), and groups combined issues under their actual file. Downloaded issue counts are not presented as file counts. Cover images and other general files are excluded.
