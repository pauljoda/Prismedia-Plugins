...
- Search Sonarr's upstream series catalog and carry its TVDB, TMDB, IMDb, artwork, and descriptive metadata into review.
...
The read-only `inspect-managed-release` operation checks the exact holding and finite monitoring scope before and after activity inspection. A holding already removed from Sonarr is confirmed through both its missing exact resource and a complete catalog that excludes the old manager ID and pinned TVDB identity; no path or configuration is invented for that absence. The operation requires a complete empty download queue, including unknown items, and terminal command states. Download-client health problems also block release: an unavailable client can make an empty queue incomplete. Health and queue APIs are observations, not a remote lock or a guarantee against a newly started download. Keep the selected scope unmonitored and avoid starting work directly during the host's handoff.
